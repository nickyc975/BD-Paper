...
基于以上的MC-R树索引结构，我们设计如下的分布式空间top-k频繁关键字查询算法：
...
\begin{itemize}
    ...
    \item[3.] 结点$N_i$收到这个查询请求后,为这个请求编号为$Q_j$,为这个请求做相应的初始化工作,包括分配一个大小为K的小根堆MinHeap,分配一个查询结点队列QueryList(用于保存需要$N_i$需要查询的索引树的Clients结点)
    \item[4.] 由结点$N_i$向索引R树的Master结点发送请求,Master结点返回索引得到的Clients结点,将这些Clients结点添加进QueryList中
    \item[4.] 向QueryList中的所有索引结点发送查询请求
\end{itemize}
\subsection{索引过程}
\begin{itemize}
    \item[1.] 对应于请求过程中的步骤3,在一个索引树的Master结点收到结点$N_i$的查询请求后,根据请求中的查询区域$R_Q$,检索自己的已有的全局索引,得到相应记录所在的Clients结点,将这些结点返回给结点$N_i$.
    \item[2.] 对应于请求过程的步骤4,在一个索引树的Clients节点收到结点$N_i$的查询请求后,根据请求中的查询区域$R_Q$,找到所有存储在自己结点的所有包含在区域$R_Q$中的记录\verb|<t, t.entries, t.freq>|,然后根据关键词t将具有相同关键词t的记录全部发向同一个计算结点$C_k$
\end{itemize}
\subsection{计算过程}
\begin{itemize}
    \item[1.] 对应于索引过程的步骤2,计算结点$C_k$在收到来自索引树的叶子结点的所有记录后,根据\verb|<t, t.entries, t.freq>|将具有相同的关键词t的记录聚集在一起,将其出现频率\verb|t.freq|累加在一起,形成新的\verb|<t, t.entries, t.freq>|
    \item[2.] 所有的$C_k$在计算完成后,将运算结果发送给负责此次查询的结点$N_i$
\end{itemize}
\subsection{响应过程}
\begin{itemize}
    \item[1.] 结点$N_i$在收到来自$C_k$的计算结果后,根据\verb|<t, t.entries, t.freq>|中的\verb|t.freq|作为排序依据,如果$t.freq>MinHeap$的顶部记录,则将记录添加进MinHeap中.否则不做处理,当收到所有负责计算的结点的计算结果后,MinHeap中的所有关键词就是最终需要的结果
    \item[2.] 结点$N_i$将MinHeap中的所有结果进行汇总和封装,返回给用户
    \item[3.] 释放此次查询所占用的全部资源,包括MaxHeap,QueryQueue,负责汇总计算的所有$C_k$
\end{itemize}
\end{document}